...
// 基础订单大小 (base_order_size): 0.001 BTC
// 最小利润 (min_profit): 0.05% (0.0005)
// 最大持仓 (max_position): 0.1 BTC
// 订单档位 (order_levels): 5
// 是否激活 (is_active): true
async function initBTCUSDTBot() {
  try {
    ...
    await db.pool.query(
      ...
      (name, account_id, trading_pair_id, base_spread, base_order_size, min_profit, max_position, order_levels, is_active, amount_precision, price_precision) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['BTC_USDT_Bot', accountId, tradingPairId, 0.001, 0.001, 0.0005, 0.1, 5, true, 6, 2]
    );

    console.log('BTC_USDT 交易对机器人初始化完成');
  } catch (error) {
    console.error('初始化 BTC_USDT 交易对机器人时发生错误:', error);
  } finally {
    await db.pool.end();
  }
}
