...
async function initLBKUSDTBot() {
  try {
    ...
    const [accountResult] = await db.pool.query(
      ...
      [process.env.CEX_NAME, process.env.CEX_API_KEY, process.env.CEX_API_SECRET, process.env.CEX_REST_API_URL,'']
    );
    ...
    // 创建配置对象
    const config = {
      base_spread: 0.01,      // 基础价差，表示买卖订单之间的最小价差百分比 1%
      base_order_size: 10,     // 基础订单大小，表示每个订单的基本数量
      min_profit: 0.01,       // 最小利润，用于决定何时更新订单的阈值 1%
      max_position: 0.1,       // 最大持仓量，用于控制风险敞口
      order_levels: 5,         // 订单档位数量，决定在每个方向上放置多少个订单
      amount_precision: 2,     // 数量精度，用于控制订单数量的小数位数
      price_precision: 9,      // 价格精度，用于控制订单价格的小数位数
    ...
    await db.pool.query(
      ...
      VALUES (?, ?, ?, ?, ?)`,
      ['BTC_USDT_BOT', accountId, tradingPairId, JSON.stringify(config), true]
    );

    // console.log('BTC_USDT_BOT 交易对机器人初始化完成');
  } catch (error) {
    console.error('初始化 BTC_USDT_BOT 交易对机器人时发生错误:', error);
  } finally {
    await db.pool.end();
  }
}
